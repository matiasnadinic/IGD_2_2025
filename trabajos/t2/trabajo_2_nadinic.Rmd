...
# Proyecto de Microsimulación Geoespacial: Alfabetización y Escolaridad en la RM

Este documento detalla el flujo de trabajo completo para la preparación,
imputación y microsimulación de datos de la encuesta CASEN 2017, con el
fin de generar estimaciones de variables a nivel de Zona Censal para la
Región Metropolitana.

## Metodología

El objetivo principal es estimar la distribución de la alfabetización
(`e1`) y la escolaridad (`esc`) a un nivel geográfico detallado (zona
censal), para el cual no tenemos encuestas directas.

El desafío es que poseemos dos fuentes de datos: 1. **Encuesta CASEN:**
Una "muestra" rica en variables (como alfabetización `e1`), pero
geográficamente pobre (solo identifica la `comuna`). 2. **Censo
(`cons_censo_df`):** Una "población" geográficamente rica (identifica la
`GEOCODIGO` o zona censal), pero pobre en variables (no tiene `e1`).

La metodología empleada es la **Microsimulación Espacial** mediante la
técnica de **Ajuste Proporcional Iterativo (IPF)**, implementada en el
paquete `rakeR`. El proceso se divide en las siguientes etapas:

### 1. Preparación e Imputación de la Muestra (CASEN)

La base de datos `casen_preparado` se crea como la "semilla" de la
simulación. \* **Imputación:** Dado que `esc` y `e1` (alfabetización)
tienen valores faltantes, se imputan usando modelos de regresión: \*
`esc` se predice usando un **GLM (Poisson)**, que es adecuado para datos
de conteo (años de escolaridad) y evita valores negativos. \* `e1` se
predice usando una **Regresión Logística Ordinal (`polr`)**, que es el
método correcto para una variable categórica ordenada (1-4). \*
**Ponderación:** Se identifica y limpia el ponderador de la encuesta
(`expr`), que es crucial para que `rakeR` entienda el peso de cada
individuo en la muestra.

### 2. Homologación de Categorías

Este es el paso más crítico para que `rakeR` funcione. \* Se identifican
las variables de control comunes entre ambas bases (sexo, edad,
escolaridad). \* Se crean nuevas columnas en la muestra CASEN
(`sexo_cat`, `edad_cat`, `esc_cat`) que replican **exactamente** los
nombres de las columnas del Censo (ej. `edad_0_30`, `esco_8_12`). \*
**Corrección Crítica:** Las columnas `_cat` (que R crea como `Factor`)
se fuerzan a tipo `character` (texto) para que coincidan perfectamente
con los nombres de columna del Censo (que son `character`), evitando el
error de `string mismatches`.

### 3. Microsimulación (Raking)

Aquí se "crean" los datos a nivel de zona censal. \*
**`rakeR::weight`:** Esta función genera una matriz de pesos. Toma la
muestra CASEN (`inds`) y los totales del Censo (`cons`) y, para *cada*
`GEOCODIGO` (zona censal), calcula un nuevo ponderador que "obliga" a la
muestra a tener los mismos totales de sexo, edad y escolaridad que esa
zona censal. \* **`rakeR::extract`:** Esta función usa la matriz de
pesos para "clonar" virtualmente a los individuos de la muestra CASEN y
asignarlos a un `GEOCODIGO`. El resultado es la
`poblacion_sintetica_df`, un dataframe masivo que ahora tiene **ambas**
cosas: las variables detalladas de CASEN (`e1`, `esc`) y la geografía
detallada del Censo (`GEOCODIGO`).

### 4. Agregación y Visualización

Con la población sintética creada, el resto del proceso es un análisis
geoespacial estándar: 1. **Agregar:** La `poblacion_sintetica_df` se
agrupa por `GEOCODIGO` y se calculan las tasas de interés (ej.
`tasa_alfabetizacion_sim`). 2. **Cargar Geometrías:** Se cargan las
geometrías de las Zonas Censales (`dpa.zonas_censales_v`) desde PostGIS,
filtrando por la Región Metropolitana (`region = '13'`). 3. **Unir:** Se
unen los datos agregados (`df_zonas_simulado`) al mapa de zonas
(`sf_zonas_rm`) usando el `GEOCODIGO` (estandarizado a 11 dígitos con
`str_pad`). 4. **Mapear:** Se generan los mapas temáticos y bivariados
finales.
...
Carga de todas las librerías necesarias para el análisis.
...
ruta_casen = here::here("data", "casen_rm.rds")
ruta_censo = here::here("data", "cons_censo_df.rds")
